...
class SimulationGUI:
    ...
    def _create_widgets(self):
        ...
        ttk.Combobox(
            ...
            values=["random", "ring", "fully_connected"],
        ).pack()
        ...
        self.run_btn = ttk.Button(
            config_frame, text="Run Simulation", command=self.start_simulation
        )
        self.run_btn.pack(pady=20)

        # Stop Button
        self.stop_btn = ttk.Button(
            config_frame, text="Stop", command=self.stop_simulation, state=tk.DISABLED
        )
    ...
    def update_plot(self, frame):
        if self.simulation_running and self.model.running:
            ...
            for i in range(len(agents)):
                self.ax.plot(
                    self.steps, self.agent_values_history[i], label=f"Agent {i}"
                )
